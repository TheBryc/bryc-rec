  ...
  (fn [{:keys [db]} [_ response]]
    (let [user {:email (:email response)}]
      {:db (-> db
       ...
       ::auth-fx/marker-set-reporter {:email (:email response)}})))
...
(rf/reg-event-fx
  ...
  (fn [{:keys [db]} [_ on-success user]]
    (cond-> {:db (-> db
                     (assoc-in [:auth :status] true)
                     (assoc-in [:auth :user] user))
             ::auth-fx/marker-set-reporter {:email (:email user)}}
      on-success (assoc ::router-fx/navigate {:route-name :hub-dashboard
