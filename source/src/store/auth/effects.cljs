...
(rf/reg-fx
  ...
  (fn [{:keys [email]}]
    (js/setTimeout
      (fn []
        (when-let [marker (.-Marker js/window)]
          (try
            (.setReporter marker #js {:email email})
            (catch js/Error e
              (js/console.warn "Marker.io setReporter error:" e)))))
      500)))

(rf/reg-fx
  ::marker-clear-reporter
  (fn [_]
    (when-let [marker (.-Marker js/window)]
      (try
        (.clearReporter marker)
        (catch js/Error e
          (js/console.warn "Marker.io clearReporter error:" e))))))
